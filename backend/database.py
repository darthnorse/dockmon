...
class Agent(Base):
    ...
    engine_id = Column(String, nullable=False, index=True)  # Docker engine ID (uniqueness gated by force_unique flag + partial index, not unconditional UNIQUE)
    ...
    # FORCE_UNIQUE_REGISTRATION opt-in (cloned-VM scenarios). When True, this row
    # is exempt from the partial unique index `idx_agent_engine_id_strict`.
    force_unique = Column(Boolean, nullable=False, server_default='0')
