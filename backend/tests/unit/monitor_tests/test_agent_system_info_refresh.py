...
def db(tmp_path):
    ...
    with db_manager.get_session() as session:
        session.add(DockerHostDB(
            id=HOST_ID, name="mediadmz", url="agent://", connection_type="agent",
            os_version="Debian GNU/Linux 12 (bookworm)", docker_version="27.0.0",
            kernel_version="6.1.0", total_memory=8_000_000_000, num_cpus=4,
        ))
        session.add(Agent(
...
async def test_refresh_asks_the_agent_and_updates_the_host_row(monitor):
    monitor._test_executor.execute_command.return_value = _result(response=NEW_INFO)

    updated = await monitor._refresh_agent_hosts_system_info()

    assert updated == 1
    call = monitor._test_executor.execute_command.await_args
...
async def test_unchanged_info_is_not_counted_as_an_update(monitor):
    monitor._test_executor.execute_command.return_value = _result(response={
        "os_type": None, "os_version": "Debian GNU/Linux 12 (bookworm)",
        "kernel_version": "6.1.0", "docker_version": "27.0.0",
        "daemon_started_at": None, "total_memory": 8_000_000_000, "num_cpus": 4,
    })
...
async def test_old_agent_without_the_command_is_quiet(monitor, caplog):
    monitor._test_executor.execute_command.return_value = _result(
        success=False, error="unknown command: get_system_info")
...
async def test_failed_command_leaves_the_row_untouched_and_warns(monitor, caplog):
    monitor._test_executor.execute_command.return_value = _result(
        success=False, error="Command timed out after 10s")

    with caplog.at_level(logging.WARNING):
        updated = await monitor._refresh_agent_hosts_system_info()

    assert updated == 0
    assert [r for r in caplog.records if "mediadmz" in r.message or AGENT_ID[:8] in r.message]
    assert _host(monitor.db).docker_version == "27.0.0"


async def test_disconnected_agent_is_skipped_without_a_command(monitor, monkeypatch):
    import agent.connection_manager as cm_module
    monkeypatch.setattr(cm_module.agent_connection_manager, "is_connected", lambda agent_id: False)

    assert await monitor._refresh_agent_hosts_system_info() == 0
    monitor._test_executor.execute_command.assert_not_awaited()


async def test_non_dict_response_is_rejected(monitor, caplog):
    monitor._test_executor.execute_command.return_value = _result(response=["not", "a", "dict"])
...
async def test_docker_host_branch_skips_agent_hosts_silently(monitor, caplog):
