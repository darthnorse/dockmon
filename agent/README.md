...
- `FORCE_UNIQUE_REGISTRATION` - Set to `true` to register this agent as a distinct host even if its Docker `engine_id` matches an already-registered host. Designed for cloned VMs / LXC templates that share `/var/lib/docker/engine-id`. **Requires `AGENT_NAME` to be set** so each cloned host has a unique display name. Skips DockMon's auto-migration from existing remote-mTLS hosts. Has no effect when set to anything other than `true`. Defaults to `false`.
